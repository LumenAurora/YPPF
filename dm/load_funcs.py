...
def create_person(name, user, **defaults):
    ...
    try:
        stage = 'get user'
        user = user if isinstance(
            user, User) else User.objects.get(username=user)
...
def get_user_by_name(name):
    ...
    """
    try:
        return NaturalPerson.objects.get(name=name).get_user()
    except:
        pass
    try:
        return Organization.objects.get(oname=name).get_user()
    except:
        pass
...
def load_stu(filepath: str, output_func: Callable = None, html=False):
    ...
    exist_list = []
    failed_list = []
    Char2Gender = {"男": NaturalPerson.Gender.MALE,
                   "女": NaturalPerson.Gender.FEMALE}
    username = 'null'
    fail_info = None
...
def load_course_record(filepath: str, output_func: Callable = None, html: bool = False) -> str:
    ...
    for i in range(4, info_height):
        ...
        if not orga_found.exists():  # 若查询不到，使用模糊查询
            orga_found = Organization.objects.filter(
                oname__contains=course_name)
        ...
        for person in info_show['person not found']:
            ...
            if person[1].exists():
                for message in person[1]:
                    ...
                    display_message += '<div style="color:cadetblue;">' + \
                        message.name + ' ' + message.person_id.username + '</div>'
    ...
    if info_show['course not found']:
        ...
        for course in info_show['course not found']:
            display_message += '<div style="color:rgb(86, 170, 142);">' + \
                course + '</div>'
...
def load_old_org_tags(filepath: str, output_func: Callable = None, html=False):
    ...
    for _, tag_dict in org_tag_def.iterrows():
        ...
        with transaction.atomic():
            try:
                useroj = Organization.objects.select_for_update().get(
                    oname=tag_dict[0])
    ...
    for _, comment_dict in feedback_df.iterrows():
        ...
        try:
            ...
            comment_time = datetime.strptime(
                comment_dict["time"], "%m/%d/%Y %H:%M %p")

            comment = Comment.objects.create(
                commentbase=feedback, commentator=commentator, text=comment_dict[
                    "text"], time=comment_time
            )
...
def load_major(filepath: str, output_func: Callable = None, html=False):
    ...
    majors = [line for i, line in enumerate(lines) if (
...
def load_minor(filepath: str, output_func: Callable = None, html=False):
    ...
    lines = [line.strip() for line in file.readlines()]
    minors = [line for i, line in enumerate(lines) if (
        line != '') and (i > 0 and lines[i-1] != '')]
    for minor in minors:
        AcademicTag.objects.get_or_create(
...
def load_birthday(filepath: str, use_name: bool = False, slash: bool = False):
    ...
    for i in range(len(csv)):
        ...
        try:
            if not slash:
                year, month, day = map(
                    int, (birthday[:4], birthday[4:6], birthday[6:]))
            else:
